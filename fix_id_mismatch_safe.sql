-- SAFE FIX FOR ID MISMATCH WITH FOREIGN KEY CONSTRAINT HANDLING
-- This script temporarily disables foreign key checks to allow the updates

-- =============================================================================
-- STEP 1: Temporarily disable foreign key constraints
-- =============================================================================

-- Disable foreign key constraint for page_content
ALTER TABLE page_content DROP CONSTRAINT IF EXISTS page_content_updated_by_fkey;

-- Disable foreign key constraint for message_templates  
ALTER TABLE message_templates DROP CONSTRAINT IF EXISTS message_templates_created_by_fkey;

-- Disable foreign key constraint for admin_messages (if exists)
ALTER TABLE admin_messages DROP CONSTRAINT IF EXISTS admin_messages_sent_by_fkey;

-- Disable foreign key constraint for scheduled_messages (if exists)
ALTER TABLE scheduled_messages DROP CONSTRAINT IF EXISTS scheduled_messages_created_by_fkey;

-- Disable foreign key constraints for demo_assignments (if exists)
ALTER TABLE demo_assignments DROP CONSTRAINT IF EXISTS demo_assignments_assigned_by_fkey;
ALTER TABLE demo_assignments DROP CONSTRAINT IF EXISTS demo_assignments_admin_user_id_fkey;

-- =============================================================================
-- STEP 2: Update the admin_users table IDs first
-- =============================================================================
...
-- =============================================================================
-- STEP 3: Now update all foreign key references in dependent tables
-- =============================================================================
...
-- Re-add foreign key constraint for page_content
ALTER TABLE page_content 
ADD CONSTRAINT page_content_updated_by_fkey 
FOREIGN KEY (updated_by) REFERENCES admin_users(id);

-- Re-add foreign key constraint for message_templates
ALTER TABLE message_templates 
ADD CONSTRAINT message_templates_created_by_fkey 
FOREIGN KEY (created_by) REFERENCES admin_users(id);

-- Re-add foreign key constraint for admin_messages (if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'admin_messages') THEN
        ALTER TABLE admin_messages 
        ADD CONSTRAINT admin_messages_sent_by_fkey 
        FOREIGN KEY (sent_by) REFERENCES admin_users(id);
    END IF;
END $$;

-- Re-add foreign key constraint for scheduled_messages (if table exists)
DO $$
BEGIN
...
-- Re-add foreign key constraints for demo_assignments (if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'demo_assignments') THEN
        ALTER TABLE demo_assignments 
        ADD CONSTRAINT demo_assignments_assigned_by_fkey 
        FOREIGN KEY (assigned_by) REFERENCES admin_users(id);
        
        ALTER TABLE demo_assignments 
        ADD CONSTRAINT demo_assignments_admin_user_id_fkey 
        FOREIGN KEY (admin_user_id) REFERENCES admin_users(id);
    END IF;
END $$;

-- =============================================================================
-- STEP 5: Handle users without auth accounts
-- =============================================================================
-- Delete unused admin users
DELETE FROM admin_users WHERE email IN ('john.doe@nexusai.com', 'jane.smith@nexusai.com');

-- =============================================================================
-- STEP 6: Verification queries to confirm the fix
-- =============================================================================
...
SELECT 
  'FOREIGN KEY CHECK - demo_assignments' as section,
  COUNT(*) as remaining_old_references
